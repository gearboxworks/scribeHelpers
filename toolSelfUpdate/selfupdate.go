...
func (su *TypeSelfUpdate) SetOldVersion(value string) *ux.State {
	for range onlyOnce {
...
func (su *TypeSelfUpdate) SetRepo(value ...string) *ux.State {
	for range onlyOnce {
		if su.OldVersion == nil {
			su.OldVersion = su.useRepo.Version
		}
...
func (su *TypeSelfUpdate) UpdateTo(newVersion *VersionValue) *ux.State {
	for range onlyOnce {
		if su.IsNotValid() {
			break
		}
...
func (su *TypeSelfUpdate) PrintVersionSummary(version string) *ux.State {
	for range onlyOnce {
		if su.IsNotValid() {
			break
		}
...
func (su *TypeSelfUpdate) PrintSummary(release *github.RepositoryRelease) *ux.State {
	for range onlyOnce {
		if su.IsNotValid() {
			break
		}
