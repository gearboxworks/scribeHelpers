...
2. Templates, (aka Tools)
	...
	- Methods are "typed" directly from the low-level types - EG TypeGit => ToolGit, TypeSystem => ToolSystem.
		- This is to avoid unnecessary type duplication.
	- Tool types have corresponding "Reflect()" functions to ease translation back and forth between low level types.
	- Any functions with a "Tool" prefix can be accessed within templates, all other functions are ignored.
		- This is to allow for functions to be "package visible", yet "template invisible".
		- EG - ToolNewGit - is available as "{{ $git := NewGit }}"
		- EG - ToolNewSystem - is available as "{{ $sys := NewSystem }}"
