...
func (p *TypeOsPath) Remove() *ux.State {
	for range OnlyOnce {
		...
		for range OnlyOnce {
			...
			if !helperPrompt.HelperUserPromptBool("Remove path '%s'? (Y|N) ", p._Path) {
...
func (p *TypeOsPath) RemoveFile() *ux.State {
	for range OnlyOnce {
		...
		for range OnlyOnce {
			...
			p.State.Clear()
			if !helperPrompt.HelperUserPromptBool("Remove file '%s'? (Y|N) ", p._Path) {
...
func (p *TypeOsPath) RemoveDir() *ux.State {
	for range OnlyOnce {
		...
		for range OnlyOnce {
			...
			if !helperPrompt.HelperUserPromptBool("Remove directory '%s'? (Y|N) ", p._Path) {
				p.State.SetWarning("not removing file '%s'", p._Path)
				break
			}
			p.State.Clear()
