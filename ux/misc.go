...
func _Printf2(format string, args ...interface{}) {
	...
	for range OnlyOnce {
		...
		//l.Title = "[color](fg:green,bg:black)Scribe[color](fg:white,bg:black)"
		//l.TextStyle = termui.NewStyle(termui.ColorYellow)
		l.Text = "[Scribe:](fg:green)[ ](fg:white)" + fmt.Sprintf(format, args...) + "[ ](fg:white)"
		l.WrapText = false
		l.SetRect(-1, -1, w, h)
		l.Border = false
		l.BorderLeft = false
...
func _Printf3(format string, args ...interface{}) {
	...
	for range OnlyOnce {
		...
		table.SetText("[Scribe:](fg:green)[ ](fg:white)" + fmt.Sprintf(format, args...) + "[ ](fg:white)")
